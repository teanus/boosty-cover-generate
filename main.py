...
def draw_symbol_with_shadow(draw, symbol, position, font, symbol_color, shadow_color, shadow_offset):
    """Рисует символ с тенью."""
    x, y = position
    draw.text((x + shadow_offset, y + shadow_offset), symbol, font=font, fill=shadow_color)
...
def is_outside_text_area(symbol_x, symbol_y, text_x, text_y, text_width, text_height, padding):
    """Проверяет, находится ли символ за пределами текстовой области."""
    return not (text_x - padding < symbol_x < text_x + text_width + padding and
                text_y - padding < symbol_y < text_y + text_height + padding)


def main():
    # Параметры изображения
    width, height = 1900, 238
    ...
    draw_text_with_shadow(draw, text, (text_x, text_y), font, text_color, shadow_color, shadow_offset)
    ...
    for _ in range(20):
        ...
        while True:
            ...
            if is_outside_text_area(symbol_x, symbol_y, text_x, text_y, text_width, text_height, padding):
                break

        draw_symbol_with_shadow(draw, symbol, (symbol_x, symbol_y), symbol_font, symbol_color, shadow_color,
                                shadow_offset)
    ...
    image_path = "teanus_cover_boosty.png"
